...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if abs(steering) > max_steering_angle:
            raise ValueError('Excceding max steering angle')
            
        if distance < 0:
            raise ValueError('Moving backward is not valid')
            
        result = robot()
        result.lenght = self.length
        ...
        # exercute motion
        turn = tan(steering2) * distance2 / result.length #beta = tan(alpha)*d/lenght
