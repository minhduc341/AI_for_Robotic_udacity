...
def estimate_next_pos(measurement, OTHER = None):
    ...
    for i in range(1, len(OTHER)):
        ...
        angles.append(atan2(OTHER[i][1] - OTHER[i-1][1],  #dy
                            OTHER[i][0] - OTHER[i-1][0])) #dx
    ...
    avg_angle = sum(turnings) / len(turnings)

    # predict the next position as current position plus the movement,
    # calculated as moving dist along the turning direction 
    x = measurement[0] + avg_dist * cos(angles[-1] + avg_angle)
    y = measurement[1] + avg_dist * sin(angles[-1] + avg_angle)
    xy_estimate = [x, y]
...
demo_grading3333(naive_next_pos, test_target)
