...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        Omega = Omega.expand(dim+2, dim+2, [0,1] + list(range(4, dim+2)), [0,1] + list(range(4, dim+2)))
        Xi = Xi.expand(dim+2, 1, [0,1] + list(range(4, dim+2)), [0])
        ...
        for b in range(2):
            Omega.value[b  ][b  ] +=  1.0 / motion_noise
            Omega.value[b+2][b+2] +=  1.0 / motion_noise
